 ...
 *         + Anpassungen: Downscale max. Kante 800px, Entfernt 6228
 * - v1.8: Feld 6304="Selfie" wieder gesetzt, Versionsanzeige in UI ergänzt und inkrementiert
 * - v1.9: Akzeptiert ausschließlich SLFT2MD.gdt als Auftragsdatei (statt "neueste *.gdt")
 */
...
$REQUEST_GDT_NAME = 'SLFT2MD.gdt';   // NUR diese Datei wird akzeptiert
...
// ----------------- request gdt (NUR SLFT2MD.gdt) -----------------
$requestPath = rtrim($dirPdf, '/') . '/' . $REQUEST_GDT_NAME;
$reqFields   = (is_file($requestPath)) ? parse_gdt($requestPath) : [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $debugBase = [
        ...
        'dir'            => $dirPdf,
        'request_gdt'    => is_file($requestPath) ? basename($requestPath) : null,
    ];
    ...
    if (!is_file($requestPath)) {
        json_out(409, [
            'status'  => 'error',
            'message' => 'Keine gültige SLFT2MD.gdt gefunden (bitte warten/refresh).',
            'debug'   => $debugBase
        ]);
    }
    ...
    if ($patId3000 === '') {
        json_out(422, [
            'status'  => 'error',
            'message' => 'Feld 3000 (Patienten-ID) fehlt in SLFT2MD.gdt',
            'debug'   => $debugBase
        ]);
    }

    // 8315/8316 gespiegelt (Request -> Antwort)
    $ans8315 = ($reqFields['8316'] ?? '') !== '' ? $reqFields['8316'] : 'T2MED_PX';
    $ans8316 = ($reqFields['8315'] ?? '') !== '' ? $reqFields['8315'] : 'KIMP_GDT';

    // Antwort-GDT bauen (OHNE 6228)
    $lines = [];
    ...
    // Danach: Auftragsdatei löschen (NUR SLFT2MD.gdt)
...
$sessionId = is_file($requestPath) ? basename($requestPath) : 'NO_REQUEST';
...
<body>
  <div class="card">
    ...
    <div class="hint">
      Warte auf Auftrag <b><?php echo h($REQUEST_GDT_NAME); ?></b> im Ordner:<br/>
